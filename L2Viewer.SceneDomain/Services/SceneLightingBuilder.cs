        ...
        return unr.ExportObjects
            ...
            .Select(x =>
            {
                ...
                return new SceneMoonData
                {
                    ...
                    Position = x.Position,
                    MakeLightmap = x.MakeLightmap,
                    MoonLight = x.MoonLight,
                    EnvironmentType = x.EnvType,
                    Directional = x.Directional,
                    LightHue = x.LightHue,
                    LightSaturation = x.LightSaturation,
                    LightBrightness = x.LightBrightness,
                    SunAffect = x.SunAffect,
                    DynamicActorFilterState = x.DynamicActorFilterState,
                    LightChanged = x.LightChanged,
                    SkinReferences = x.Skins.Select(reference => BuildMapReference(unr, reference)).Where(x => x is not null).Cast<string>().ToArray(),
                    FlameReferences = x.Flames.Select(reference => BuildMapReference(unr, reference)).Where(x => x is not null).Cast<string>().ToArray()
                };
            })
            .OrderBy(x => x.Name, StringComparer.OrdinalIgnoreCase)
            .ToArray();
